...
do_sync() {
    ...
    sudo chown -R cirrus:cirrus /usr/local/lib/python3.12/dist-packages || true
...
do_upload() {
    cd brave-browser
    APKSIGNER=$(find src/third_party/android_sdk/public/build-tools -name apksigner -type f | head -n 1)
    mkdir -p src/out/Release/apks/signed
    INPUT_APK="src/out/Release/apks/BravePublic.apk"
    OUTPUT_APK="src/out/Release/apks/signed/Brave-Clean.apk"
